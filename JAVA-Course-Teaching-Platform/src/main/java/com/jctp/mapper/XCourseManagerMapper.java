...
public interface XCourseManagerMapper {
	@Select("SELECT fil.id,fil.class_id,cla.class_no,cou.work_no,fil.teacher_id,fil.course_name,fil.create_date,fil.grade,fil.term,fil.course_img,cou.name AS teachername FROM JCTP_COURSE fil left join JCTP_USER cou ON fil.teacher_id=cou.id"
			+ " left join JCTP_CLASS cla on fil.class_id=cla.id"
			+ " WHERE  course_name LIKE CONCAT('%',#{param.courseName},'%') and cou.work_no LIKE CONCAT('%',#{param.workNo},'%') and cla.class_no LIKE CONCAT('%',#{param.classNo},'%')"
